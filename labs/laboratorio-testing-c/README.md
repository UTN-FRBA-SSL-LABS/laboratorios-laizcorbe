...
> R:Porque inicializa el carrito vacio, es decir, en cero. 
...
> R:Dice que fallo, que esperaba 700, y obtuvo 350
...
> R:En la linea de carrito_total hay un bug. 

**P7** — ¿Qué debería hacer esa línea para calcular el total correctamente?

> R: Le falta multiplicar el precio por la cantidad de productos que se selecciona.
...
> R: En la primera ejecucion el test falla porque carrito_agregar devolvia 1 al intentar agregar un quinto elemento. Porque no estaba puesta la condicion, permitia que entrar un elemento fuera del rango esperado. 
...
> R:El operador incorrecto era: <=   Tenia que ser estrictamente menor (<), porque si no tomaba 0, 1, 2, 3, 4  
...
> R:Si, carrito_descuento porque es una funcion que no llamamos dentro de test unitarios
...
> R:Un test UNITARIO prueba una funcion de una manera independiente, mientras que un test de INTEGRACION verifica como interactuan varias funciones combinadas entre si. 
...
> R: Escribir por fuera de un array accede a zonas de la memoria que no se deberia.  
...
> R: Escribirs los test permite verificar el comportamiento de forma objetiva y ejecutable. Permite inspeccionar mas el codigo y tambien aislar el problema en caso de que lo haya.  
...
> R:Porque cuando la cantidad es 1, la multiplicacion (precio x 1) da el mismo valor que el precio solo, por lo que falla la formula. 
